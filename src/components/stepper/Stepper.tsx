...
const Stepper: FC<IStepper> = ({steps, currentStep}) => {
 ...
 const updateStep = (stepNumber: number, steps: any) => {
  ...
  const displaySteps = newStep.map((step: any, index: number) => {
    return (
      <div key={index} className={ index !== newStep.length - 1 ? "w-full h-[70vh] flex flex-col items-start" : 'flex flex-col items-center'}>
        <div className="relative flex  items-center text-n60 text-xs">
          ...
          <div className={`absolute top-0 text-center  w-32 font-medium uppercase ${step.highlighted ? "text-gray-900": "text-gray-400"}` }>
            {/* Display Description */}
            {step.description}
          </div>
        </div>
  ...
  return (
    <div className="mx-4 p-4 h-[70vh] flex flex-col justify-between items-start">
      {displaySteps}
    </div>
  )
}

...
const renderUserProfile = (item: { buttonFunc: React.MouseEventHandler<HTMLDivElement> | undefined | any; icon: JSX.IntrinsicAttributes; content: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined; }, index: React.Key | null | undefined) => (
  <div onClick = {item.buttonFunc}>
    <Link href = "/" key={index} >
    ...
    <>
      <SidebarWrapper>
        ...
        <div className='relative flex flex-col justify-between  h-full'>
          ...
          {/* Sidebar Listing for Company */}
          {/* <MenuItemsList options={COMPANY_ITEMS :  userType === 'user' &&  MENU_ITEMS} />             */}

          <MenuItemsList options={MENU_ITEMS} />
          <MenuItemsList options={MENU_ITEMS2} />
          {/* <MenuItemsList options={MENU_ITEMS} />             */}
          {/* Right Hand side nav */}
          <div className = "relative flex items-center justify-center right_Topnav h-20 mb-20">
            <Dropdown
              customToggle = {() => RenderUserToggle()}
              contentData = {user_menu}
              renderData = {(item: any, index: any) => renderUserProfile(item, index)}
            />
          </div>
        </div>
      </SidebarWrapper>
